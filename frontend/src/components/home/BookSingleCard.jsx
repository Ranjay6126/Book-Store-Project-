...
const BookSingleCard = ({ book, index = 0 }) => {
  ...
  return <><article className="animate-fade-up relative overflow-hidden rounded-lg border border-[#bdcfb9] bg-[#fffef9] transition hover:-translate-y-0.5 hover:shadow-lg"><div className="absolute inset-y-0 left-0 w-1" style={{ backgroundColor: shade }} /><div className="p-5 pl-6"><div className="flex gap-4"><div className="grid h-28 w-20 shrink-0 place-items-center rounded-sm text-[#fffdf7] shadow-md" style={{ backgroundColor: shade }}>{book.coverImage ? <img src={book.coverImage} alt={`${book.title} cover`} className="h-full w-full rounded-sm object-cover" /> : <PiBookOpenTextLight className="text-3xl" />}</div><div className="min-w-0"><p className="eyebrow !text-black">{book.publishYear}</p><h2 className="mt-2 line-clamp-2 font-serif text-xl font-bold leading-tight text-black">{book.title}</h2><p className="mt-2 text-sm font-medium text-black">{book.author}</p>{book.genre && <p className="mt-3 text-xs font-bold tracking-wide text-black uppercase">{book.genre}</p>}</div></div>{book.description && <p className="mt-5 line-clamp-2 text-sm leading-6 text-black">{book.description}</p>}<div className="mt-5 flex items-center border-t border-[#cbdac7] pt-3"><button type="button" onClick={() => setShowModal(true)} className="mr-auto inline-flex min-w-28 items-center justify-center gap-1.5 rounded border border-[#8eaa8b] bg-[#edf8e9] px-3 py-2 text-sm font-bold text-black shadow-sm transition hover:bg-[#d9eed3]"><BiShow /> Preview</button><SmallAction to={`/books/details/${book._id}`} title="Details"><BsInfoCircle /></SmallAction><SmallAction to={`/books/edit/${book._id}`} title="Edit"><AiOutlineEdit /></SmallAction><SmallAction to={`/books/delete/${book._id}`} title="Delete" danger><MdOutlineDelete /></SmallAction></div></div></article>{showModal && <BookModal book={book} onClose={() => setShowModal(false)} />}</>;
};
