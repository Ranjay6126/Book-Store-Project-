...
const Home = () => {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-11">
    <section className="animate-fade-up border-b border-[#b9cbb4] pb-6"><h1 className="flex items-center gap-2 font-serif text-4xl font-bold tracking-tight text-[#17231a] sm:text-5xl"><PiBookOpenTextLight className="shrink-0 text-3xl sm:text-4xl" />Your library Collection</h1><div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><p className="max-w-3xl text-sm font-medium leading-6 text-[#1e3024]">The Book Store System allows users to view, add, edit, and delete books. A confirmation warning is shown before deletion, as deleted books cannot be recovered.</p><div className="shrink-0"><p className="mb-2 text-center text-sm font-bold text-black">{loading ? "Loading books…" : `${visibleBooks.length} ${visibleBooks.length === 1 ? "title" : "titles"}`}</p><div className="flex flex-wrap items-center gap-2"><div className="flex min-w-[278px] justify-center gap-2 rounded-md border border-[#94ac91] bg-[#dcefd8] p-1"><ViewButton active={showType === "table"} onClick={() => setShowType("table")} label="Table view" /><ViewButton active={showType === "card"} onClick={() => setShowType("card")} label="Card view" /></div><Link to="/books/create" className="btn-primary rounded-full text-sm"><MdOutlineAdd className="text-lg" /> Add book</Link></div></div></div></section>
