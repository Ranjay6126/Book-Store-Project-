...
const Home = () => {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-11">
    <section className="animate-fade-up border-b border-[#9fbc99] pb-6"><h1 className="flex items-center gap-2 font-serif text-4xl font-bold tracking-tight text-[#17231a] sm:text-5xl"><span className="grid h-9 w-9 place-items-center rounded-lg border border-[#759971] bg-[#c7dfc0] text-2xl text-black sm:h-11 sm:w-11 sm:text-3xl"><PiBooksLight /></span>Your library Collection</h1><div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><p className="max-w-3xl text-sm font-medium leading-6 text-[#1e3024]">The Book Store System allows users to view, add, edit, and delete books. A confirmation warning is shown before deletion, as deleted books cannot be recovered.</p><div className="shrink-0"><p className="mb-3 text-center text-sm font-bold text-black">{loading ? "Loading books…" : `${visibleBooks.length} ${visibleBooks.length === 1 ? "title" : "titles"}`}</p><div className="flex flex-wrap items-center gap-2"><div className="flex min-w-[278px] justify-center gap-2 rounded-md border border-[#94ac91] bg-[#dcefd8] p-1"><ViewButton active={showType === "table"} onClick={() => setShowType("table")} label="Table view" /><ViewButton active={showType === "card"} onClick={() => setShowType("card")} label="Card view" /></div><Link to="/books/create" className="btn-primary rounded-full text-sm"><MdOutlineAdd className="text-lg" /> Add book</Link></div></div></div></section>
    ...
    <section className="animate-fade-up mt-4 grid gap-3 rounded-lg border border-[#dedbd0] bg-[#fffef9] p-3 sm:grid-cols-[1fr_220px]"><label className="relative"><HiOutlineSearch className="absolute top-1/2 left-3 -translate-y-1/2 text-lg text-[#7b877d]" /><input className="field pl-10" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title, author, or year" aria-label="Search books" /></label><select className="field cursor-pointer" value={sortKey} onChange={(event) => setSortKey(event.target.value)} aria-label="Sort books">{Object.entries(SORTS).map(([key, sort]) => <option key={key} value={key}>{sort.label}</option>)}</select></section>
...
const Stat = ({ value, label }) => <div className="px-4 py-4 sm:px-6"><p className="font-serif text-2xl font-bold text-[#203125] sm:text-3xl">{value}</p><p className="mt-1 text-[11px] font-bold tracking-[.11em] text-[#33463a] uppercase">{label}</p></div>;
